locals {
  lb_web_hosts       = ["ascoor.app", "www.ascoor.app"]
  lb_api_hosts       = ["api.ascoor.app"]
  lb_web_ssl_domains = ["ascoor.app", "www.ascoor.app"]
  lb_api_ssl_domains = ["api.ascoor.app"]
  ssl_cert_suffix    = "20260126"
}
...
resource "google_compute_url_map" "api_lb" {
  ...
  default_url_redirect {
    host_redirect          = local.lb_api_hosts[0]
    https_redirect         = true
    strip_query            = false
...
resource "google_compute_managed_ssl_certificate" "web" {
  name = "${local.app_name}-web-ssl-cert-${local.environment}-${local.ssl_cert_suffix}"
...
resource "google_compute_managed_ssl_certificate" "api" {
  name = "${local.app_name}-api-ssl-cert-${local.environment}-${local.ssl_cert_suffix}"
