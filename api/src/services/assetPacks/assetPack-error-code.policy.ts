import { AssetPackQuotaExceededError, NotFoundError } from '../../usecases/errors';

export function inferAssetPackErrorCode(error: unknown, fallback: string): string {
  const text = typeof error === 'string' ? error : ((error as Error)?.message ?? String(error));
  const normalized = text.toLowerCase();

  if (
    error instanceof AssetPackQuotaExceededError ||
  ...
  if (
    ...
    normalized.includes('enqueue pack generation task')
  ) {
    return 'TASK_ENQUEUE_FAILED';
  }
  if (normalized.includes('failed to generate an asset pack title')) {
  ...
  if (
    ...
    return 'AI_AGENT_ASSET_PACK_FAILED';
  }
  if (normalized.includes('asset pack not found')) {
    return 'ASSET_PACK_NOT_FOUND';
  }
